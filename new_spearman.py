...
v2.0 | Enhanced TAM Questionnaire (42 ερωτήσεις)
=============================================================

ΑΛΛΑΓΕΣ v2.0 vs v1.0:
  ✓ Reverse-coded items (Podsakoff et al., 2003)
  ✓ McDonald's ω (omega) — ισχυρότερο από Cronbach's α
  ✓ Corrected Item-Total Correlations (CITC)
  ✓ Alpha/Omega-if-Item-Deleted
  ✓ Bootstrap 95% CIs για μέσους όρους (n=1000 iterations)
  ✓ Effect sizes (rank-biserial r) για Wilcoxon
  ✓ Kruskal-Wallis + Dunn post-hoc για εμπειρία
  ✓ IQR στην περιγραφική στατιστική
  ✓ Auto-detection πιθανών reverse items (diagnostic)
  ✓ Inter-item correlation matrices

ΑΛΛΑΓΕΣ v2.1 vs v2.0 (βιβλιογραφική ενίσχυση):
  ✓ Common Method Bias (Harman's single factor test)
  ✓ Ceiling effects analysis (skewness, kurtosis, frequency tables)
  ✓ Average Variance Extracted (AVE) — convergent validity
  ✓ Composite Reliability (CR) — alternative to alpha
  ✓ HTMT discriminant validity (Henseler et al., 2015)
  ✓ Fornell-Larcker criterion (secondary check)
  ✓ Effect sizes for Kruskal-Wallis (η² and ε²)
  ✓ Post-hoc power analysis (minimum detectable effect)
...
# ══════════════════════════════════════════════════════════════════════════════
# 1. REVERSE-CODED ITEMS CONFIGURATION
#
#    Οι παρακάτω ερωτήσεις έχουν διατυπωθεί αρνητικά στο ερωτηματολόγιο
#    (ο ερωτώμενος πρέπει να «Διαφωνεί» για να δείξει θετική στάση).
#    Πριν κάθε ανάλυση εφαρμόζεται: recoded = 6 − raw_score
#
#    ΒΑΣΙΣΜΕΝΟ ΣΤΟ ΕΡΩΤΗΜΑΤΟΛΟΓΙΟ (erwtimatologio_ptixiakis.pdf) +
#    ΑΥΤΟΜΑΤΗ ΑΝΙΧΝΕΥΣΗ ΑΠΟ ΑΡΝΗΤΙΚΕΣ INTER-ITEM CORRELATIONS
#
#    ▶ ΕΛΕΓΞΕ ΚΑΙ ΔΙΟΡΘΩΣΕ ΑΝ ΧΡΕΙΑΣΤΕΙ:
#      Κοίτα την ενότητα "REVERSE ITEMS DIAGNOSTIC" στα αποτελέσματα.
#      Αν μια ερώτηση εμφανίζεται με κόκκινο αστερίσκο (*), πιθανόν 
#      χρειάζεται reverse coding.
# ══════════════════════════════════════════════════════════════════════════════

REVERSE_ITEMS = [
    # ──────────────────────────────────────────────────────────────────────────
    # ΕΠΙΒΕΒΑΙΩΜΕΝΑ REVERSE ITEMS (από τον ερευνητή)
    # Τύπος recoding: recoded = 6 − raw_score  (Podsakoff et al., 2003)
    # ──────────────────────────────────────────────────────────────────────────

    "Q17",   # PEOU (Q15–Q18) — Construct: Perceived Ease of Use
             # Πραγματική ερώτηση:
             #   «Η χρήση εργαλείων τεχνητής νοημοσύνης συχνά με δυσκολεύει
             #    περισσότερο από όσο με βοηθά»
             # Λογική reverse: Agree(5) = δυσκολεύει = ΧΑΜΗΛΗ ευχρηστία
             # Μετά recoding: 5→1 (δύσκολο), 1→5 (εύκολο) ✓
             # Αποτέλεσμα χωρίς recoding: α_PEOU = -0.19 (άκυρο)

    "Q23",   # Trust in AI (Q22–Q24) — Construct: Εμπιστοσύνη στην AI
             # Πραγματική ερώτηση:
             #   «Συχνά αμφισβητώ την αξιοπιστία των αποτελεσμάτων
             #    που παράγει το AI»
             # Λογική reverse: Agree(5) = αμφισβήτηση = ΧΑΜΗΛΗ εμπιστοσύνη
             # Μετά recoding: 5→1 (χαμηλή εμπιστοσύνη), 1→5 (υψηλή) ✓
             # Αποτέλεσμα χωρίς recoding: α_Trust = -1.07 (εξαιρετικά άκυρο)

    # ──────────────────────────────────────────────────────────────────────────
    # ΑΛΛΑ ΠΙΘΑΝΑ REVERSE ITEMS (αν εντοπιστούν από το diagnostic):
    # Ελέγξτε το αρχείο: results/diagnostic_reverse_items.csv
    # ──────────────────────────────────────────────────────────────────────────
]
...
#    Τύπος: recoded = 6 − raw   (για 1–5 Likert)
# ══════════════════════════════════════════════════════════════════════════════
...
#    Εκτελείται πάνω στα ΠΡΩΤΟΤΥΠΑ (pre-reverse) δεδομένα.
# ══════════════════════════════════════════════════════════════════════════════
...
#
#  Two constructs showed insufficient internal consistency (low α/ω), so their
#  composite scores are unreliable. Instead, each item is correlated individually
#  against the composite scores of the 8 robust constructs.
#
#  Low-alpha constructs (9 items analyzed individually):
#   • Ποιότητα Εξόδου              (Output_Quality)            → Q25, Q26, Q27
#   • Βελτίωση Ποιότητας Εργασίας  (Work_Quality_Improvement)  → Q34, Q35, Q36
#   • Ανάγκη Ανθρώπινης Επίβλεψης  (Human_Oversight)           → Q40, Q41, Q42
#
#  Robust composites: 7 constructs (Work_Quality_Improvement excluded).
# ══════════════════════════════════════════════════════════════════════════════
...
# ── 1. Define low-alpha items ─────────────────────────────────────────────────
# Keys = column names in df; values = short label for display.
# VERIFY: Human_Oversight uses Q40–Q42 per CONSTRUCT_MAP (not Q30–Q35).
LOW_ALPHA_ITEMS = {
...
# ── 2. Define robust composite constructs (the 7 with acceptable reliability) ─
# Work_Quality_Improvement excluded — its items (Q34–Q36) are analyzed above.
# These keys must exist in the construct_scores dict computed in Section 6.
ROBUST_CONSTRUCTS = {
    "Perceived_Usefulness":  "Perceived Usefulness (PU)",
    "Perceived_EaseOfUse":   "Perceived Ease of Use (PEOU)",
    "Behavioral_Intention":  "Behavioral Intention (BI)",
...
# ══════════════════════════════════════════════════════════════════════════════
# 12. DIVERGING LIKERT CHARTS (CONSOLIDATED: 2 Multi-Panel Figures)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# 13. WILCOXON — MANN-WHITNEY U (AI Users vs Non-users)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# 14. KRUSKAL-WALLIS για ΕΜΠΕΙΡΙΑ (+ Dunn post-hoc)
#     Κατάλληλο για n=28 με ≥3 ομάδες (αντί ANOVA, Hecke & Hoffelder, 2020)
# ══════════════════════════════════════════════════════════════════════════════
...
# Kruskal-Wallis: approximate minimum detectable effect
# Using η² thresholds: small=0.01-0.06, moderate=0.06-0.14, large≥0.14
# With k=4 groups and n=30 (n_per_group≈7.5), minimum moderate effect is ≈0.08
kw_eta_min = 0.08
power_results.append({
...
# ══════════════════════════════════════════════════════════════════════════════
# CONSOLIDATE CSV OUTPUTS TO REDUCE FILE COUNT (Target: 20 files total)
# ══════════════════════════════════════════════════════════════════════════════
...
# Keep essential individual files (already saved)
# Don't resave the ones we're consolidating - just keep reference
kept_files = [
...
redundant_files = [
    ...
    "results/ave_cr_reliability.csv",
    "results/wilcoxon_tests.csv",
    # Old individual Likert charts (replaced by consolidated part1/part2)
...
# Rename and consolidate remaining files to use consistent numbering
final_renames = {
